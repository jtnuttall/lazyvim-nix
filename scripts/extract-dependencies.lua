...
local function should_exclude_tool(tool_name)
	...
	return excluded_lookup[tool_name]
		or tool_name:match("^%u+$") -- All caps (like RUFF_TRACE)
		or tool_name:match("^[a-z_]+$") and #tool_name < 3 -- Very short generic names (only 1-2 chars)
end
...
local function extract_executables_from_content(content, relative_path)
	...
	if #unique_executables > 0 then
		print(
			string.format(
				"  Found %d executables in %s: %s",
				#unique_executables,
				relative_path,
				table.concat(unique_executables, ", ")
			)
		)
	end
...
local function verify_nixpkg_exists(nixpkg_path)
	...
	local cmd = string.format(
		"nix-instantiate --eval --expr 'with import <nixpkgs> {}; %s.name or \"%s\"' 2>/dev/null",
		nixpkg_path,
		nixpkg_path
	)
...
local function extract_dependencies(lazyvim_path, mason_path, output_file, extras_entries, cache_ops)
	...
	if mason_path then
		...
		for _ in pairs(runtime_deps) do
