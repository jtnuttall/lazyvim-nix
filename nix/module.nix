...
with lib; let
  ...
  getEnabledExtras = extrasConfig: let
    flattenExtras = categoryName: prefix: node: let
      ...
      nestedList =
        ...
        metaChildren;
    in
      current ++ builtins.concatLists nestedList;

    processCategory = categoryName: categoryExtras: let
      flatUserExtras = builtins.concatLists (lib.mapAttrsToList (
          topName: topConfig: let
      ...
      resolvedExtras =
        ...
        flatUserExtras;
    in
      builtins.filter (x: x != null) resolvedExtras;
  ...
  enabledExtras =
    if cfg.enable
    then getEnabledExtras (cfg.extras or {})
    else [];
  ...
  userPlugins =
    if cfg.enable
    then fileLib.scanUserPlugins "${config.home.homeDirectory}/.config/${cfg.appName}"
    else [];
  ...
  resolvedTreesitterPlugin = let
    tsPlugins =
      lib.zipListsWith (
        spec: plugin:
          if spec.name == "nvim-treesitter/nvim-treesitter"
          then plugin
          else null
      )
      allPluginSpecs
  ...
  in
    if found != null
    then found
    else pkgs.vimPlugins.nvim-treesitter;
  ...
  treesitterGrammars =
    if cfg.pluginSource == "latest" && treesitterLib.hasParserRevisions
    then treesitterLib.treesitterGrammarsFromSource automaticTreesitterParsers
    else treesitterLib.treesitterGrammars automaticTreesitterParsers;
    ...
    xdg.configFile =
      {
        ...
        "${cfg.appName}/lua/config/autocmds.lua" =
          mkIf (
            ...
            if scannedFiles.configFiles ? autocmds
            then {source = scannedFiles.configFiles.autocmds.file;}
            else {
              text = ''
        ...
        "${cfg.appName}/lua/config/keymaps.lua" =
          mkIf (
            ...
            if scannedFiles.configFiles ? keymaps
            then {source = scannedFiles.configFiles.keymaps.file;}
            else {
              text = ''
        ...
        "${cfg.appName}/lua/config/options.lua" =
          mkIf (
            ...
            if scannedFiles.configFiles ? options
            then {source = scannedFiles.configFiles.options.file;}
            else {
              text = ''
